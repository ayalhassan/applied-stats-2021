...
f12 %>% 
  group_by(age_group) %>% 
  summarise(mean_time_to_second_birth = mean(next.ivl), max_time_to_second_birth = max(next.ivl))
```


```{r}
f12 %>% 
  ggplot(aes(next.ivl, fill = age_group)) + 
  geom_histogram(position = 'dodge', aes(y = ..density..)) + 
  labs(title = "Distribution of time to second birth by mother's age group", x = "time (years)")
```
...
f12_alt <- f12 %>% 
  ...
  summarise(events = sum(event),
            people = n()) %>% 
  mutate(cumulative_people_lost = cumsum(people),
         n_i = lag(n - cumulative_people_lost, default = n),
         prob_death = events/n_i,
         prob_surv = 1- prob_death,
         surv = cumprod(prob_surv))
...
```{r}
tibble(pch = exp(coef(fit_pois)), data = D_k/E_k)
```
...
```{r}
sample_hazards <- function(log_lambdas, ses, nsims = 100){
  log_lambdas.st <- matrix(NA, nrow = nsims, ncol = length(lambdas))
  for(i in 1:length(lambdas)){
    log_lambdas.st[,i] <- rnorm(nsims, mean = log_lambdas[i], sd = ses[i])
  }
  return(log_lambdas.st)
}

ses <- sqrt(diag(vcov(fit_pois)))

log_lambdas.st <- sample_hazards(log(lambdas), ses, nsims = 100)

df_surv_q <- bind_cols(survival_prob(exp(apply(log_lambdas.st, 2, mean)), cuts) %>% rename(mean = surv),
          survival_prob(exp(apply(log_lambdas.st, 2, quantile, 0.025)), cuts) %>% select(-time) %>% rename(upper = surv),
          survival_prob(exp(apply(log_lambdas.st, 2, quantile, 0.975)), cuts) %>% select(-time) %>% rename(lower = surv))

df_surv_q %>% 
  ggplot(aes(time, mean)) + geom_line() + 
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2)
```
...
survival_prob(lambdas_young, cuts) %>% filter(time==5) %>% arrange(-time)
```
